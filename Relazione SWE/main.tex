...
%=========================================================================================================================================
	...
	\begin{table}[h]
		...
		\begin{tabular}{|l p{12cm}|}
			...
			\textbf{Basic Flow} & \begin{enumerate}[label=\arabic*), leftmargin=*, before=\vspace{-0.7cm}, after=\vspace{-0.4cm}]
									  ...
									  \item[]
			\end{enumerate} \\
			\textbf{Alternative Flow} & 3.a) Sul sensore segnalato è già stato aperto un ticket, quindi il sistema restiturà un messagio di errore e non sarà aperto un ulteriore ticket. \\
			\textbf{Post-Conditions} & Il ticket è stato registrato. Il sistema blocca il sensore segnalato e riporta l'Admin sulla sua dashboard, indipendentemente che l'apertura del ticket sia andata a buon fine o no. \\
	...
	\begin{table}[h]
		...
		\begin{tabular}{|l p{12cm}|}
			...
			\textbf{Pre-Conditions} & Il manutentore deve essere autenticato e nella pagina "Visualizza ticket aperti".\\
			\textbf{Basic Flow} & \begin{enumerate}[label=\arabic*), leftmargin=*, before=\vspace{-0.7cm}, after=\vspace{-0.4cm}]
			...
			\textbf{Post-Conditions} & Il sistema imposta nel database il ticket come "preso" e gli assegna l'id del manutentore che l'ha selezionato. \\
	...
	\begin{table}[h]
		...
		\begin{tabular}{|l p{12cm}|}
			...
			\rowcolor{gray!25}
			\textbf{Use Case \#7} & \textbf{Lettura dello storico delle misurazioni (User)} \\
			\textbf{Brief Description} & L'utente legge le misurazioni effettuate dai sensori inserendo una data di inizio e una data di fine \\
			\textbf{Level} & User Goal \\
			\textbf{Actors} & User \\
			\textbf{Pre-Conditions} & L'utente è autenticato e sulla pagina di lettura dati \\
			\textbf{Basic Flow} & \begin{enumerate}[label=\arabic*), leftmargin=*, before=\vspace{-0.7cm}, after=\vspace{-0.4cm}]
									  \item L'utente seleziona l'opzione "Leggi storico misurazioni"
									  \item L'utente inserisce la data di inizio
									  \item L'utente inserisce la data di fine
									  \item Il sistema mostra tutte le misurazioni fatte da tutti i sensori in quell'arco di tempo
									  \item[]
			\end{enumerate} \\
			\textbf{Alternative Flow} & 2/3.a) Il sistema mostra un errore se il formato della data non è valido e permette di reinserire subito tale data\\
		...
		\vspace{0.2cm}
		\caption{Use Case \#7 (User: Lettura storico misurazioni)}
	\end{table}
	\vspace{1cm}
	...
	\begin{table}[h]
		...
		\begin{tabular}{|l p{12cm}|}
			...
			\textbf{Basic Flow} & \begin{enumerate}[label=\arabic*), leftmargin=*, before=\vspace{-0.7cm}, after=\vspace{-0.4cm}]
									  \item L'utente seleziona l'opzione "Imposta Alert Rule"
									  \item L'utente seleziona uno dei 3 tipi di Alert Rule: solo limite inferiore, solo limite superiore oppure entrambi
									  \item L'utente crea la regola impostando il tipo di sensore e il/i limite/i. Il sistema verificherà automaticamente questa regola per ogni misurazione, ad opera dei sensori di quel tipo, successiva alla creazione
									  \item Il sistema registra la regola nel database
									  \item[]
			\end{enumerate} \\
			\textbf{Alternative Flow} & 3.a) Il sistema mostra un errore perché qualche valore della regola non è valido e e permette di reinserirlo subito\\
			\textbf{Post-Conditions} & L'utente viene riportato alla dashboard iniziale \\
	...
	\begin{table}[h]
		...
		\begin{tabular}{|l p{12cm}|}
			...
			\rowcolor{gray!25}
			\textbf{Use Case \#9} & \textbf{Lettura dello storico delle notifiche (User)} \\
			\textbf{Brief Description} & L'utente legge tutte le notifiche risalenti ad un certo numero di giorni fa \\
			\textbf{Level} & User Goal \\
			\textbf{Actors} & User \\
			\textbf{Pre-Conditions} & L'utente è autenticato e sulla pagina "Visualizza notifiche non lette" \\
			\textbf{Basic Flow} & \begin{enumerate}[label=\arabic*), leftmargin=*, before=\vspace{-0.7cm}, after=\vspace{-0.4cm}]
									  \item L'utente seleziona l'opzione "Leggi storico notifiche"
									  \item L'utente inserisce il numero di giorni antecedenti la data odierna dei quali vuole vedere le notifiche
									  \item Il sistema mostra tutte le notifiche appartenenti a quell'arco di tempo
									  \item[]
			\end{enumerate} \\
			\textbf{Alternative Flow} & 2.a) Il sistema mostra un errore se l'input non è valido e permette di reinserire subito il valore\\
			\textbf{Post-Conditions} & Il sistema deve mostrare tutte e solo le notifiche appartenenti a quell'arco di tempo \\

			\hline
		\end{tabular}

		\vspace{0.2cm}
		\caption{Use Case \#9 (User: Lettura storico notifiche)}
	\end{table}
	\vspace{1cm}
%==========================================================================================================================================================
	\clearpage
	...
	Si tratta di un insieme di controller che permettono agli attori di usufruire dei servizi a loro concessi. Il \texttt{SensorManager} e il \texttt{SensorMonitor} implementano il  \textbf{Design Pattern Observer}. È anche presente una classe \texttt{AdminExtraController} utile per alcune funzioni di gestione del db (usate soprattutto nel testing). È anche presente un classe \texttt{DatabaseMutex} che contiene un Semaphore utile per la sincronizzazione dei thread Main, SystemMonitor e SensorManager.
	\begin{figure}[H]
		\centering
	...
	\subsubsection{ORM}
	Contiene un oggetto nameClassDAO per ogni classe del domain model presente nel db di sistema. \texttt{DatabaseManager} è la classe che si occupa di erogare e chiudere la connessione al database.
	\begin{figure}[H]
		\centering
...
(
	...
	\end{lstlisting}
	\newline
	Inoltre, a causa della \textbf{relazione circolare} che c'è tra le tabelle Sensor e Measurement, si è reso necessario impostare la chiave esterna dopo la creazione di entrambe le tabelle:
	\begin{lstlisting}[style = stileSQL]
	...
	\begin{itemize}
		...
		\item \textbf{Dashboard Admin}: Può visualizzare tutti gli utenti e può leggere lo storico delle misurazioni. Sempre mediante una richiesta y/n sarà chiesto se si desidera bloccare un utente oppure aprire un ticket per un sensore.
	...
	Classe che rappresenta l'oggetto che sarà inviato allo User la cui Alert Rule è appena stata violata dalle nuove misurazioni. Tra i campi, possiede il flag \texttt{isRead} che, finché sarà \texttt{false}, farà sì che la notifica in questione appaia all'utente nel contenitore delle notifiche non lette.
	...
	\end{lstlisting}
	\subsection{Domain Model}
	Di questo package è stato realizzato sollo il testing per il metodo \texttt{isViolatedBy(...)} della classe AlertRule, che verifica se una misurazione ha violato i suoi bound.
