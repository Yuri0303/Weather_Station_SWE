...
public class SensorMonitor implements Observer {
    ...
    private ArrayList<Notification> createNotifications(ArrayList<AlertRule> ars, Measurement measurement, SensorType sensorType) {
        ...
        for (AlertRule a : ars) {
            ...
            notifications.add(new Notification(0, message, LocalDateTime.now(), false, a.getUserId())); //FIXME: id=0 perché sarà l'inserimento nel database (tramite NotificationDAO) a impostarlo
        }
